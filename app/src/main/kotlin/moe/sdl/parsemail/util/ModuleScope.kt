package moe.sdl.parsemail.util
...
open class ModuleScope(
  ...
  exceptionHandler: (CoroutineContext, Throwable, KLogger, String) -> Unit = { _, e, _, _ -> logger.error(e) { "Caught Exception on $moduleName" } }
) : CoroutineScope {

  val parentJob = SupervisorJob(parentContext[Job])

  override val coroutineContext: CoroutineContext =
    parentContext + parentJob + CoroutineName(moduleName) + dispatcher + CoroutineExceptionHandler { context, e ->
